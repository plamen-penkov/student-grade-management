...
void connectDB() {
    ...
    try {
        ...
        sql = "CREATE TABLE IF NOT EXISTS grades (id INTEGER PRIMARY KEY AUTOINCREMENT, subject TEXT CHECK (subject IN ('ENGLISH', 'MATHEMATICS', 'PHYSICAL_EDUCATION', 'PHYSICS', 'CHEMISTRY', 'BIOLOGY')), grade_score NUMBER, student_id INTEGER, FOREIGN KEY (student_id) REFERENCES students (id) ON DELETE CASCADE);";
        statement.execute(sql);
...
void printStudentGrades() {
    try {
        ...
        List<Grade> studentGrades = Grade.getGradesByStudentId(student.getId(), conn);
...
void printStudentAverage() {
    try {
        ...
        Student student = Student.getById(studentId, conn);
        if (student == null) {
            System.out.println("There is no student with this ID.");
            return;
        }
        System.out.printf("%d: %s %s\n", student.getId(), student.getFirstName(), student.getLastName());
        System.out.printf("Average: %.2f%n\n", student.getGradeAverage(conn));
        System.out.println();
    } catch (SQLException e) {
        System.out.println("An error occurred.");
        System.out.println(e.getMessage());
...
void addStudentGrade() {
    try {
        System.out.printf("Enter a student id(1 - %d):", Student.getStudentCount(conn));
        int studentId = Integer.parseInt(scanner.nextLine());
        Student student = Student.getById(studentId, conn);
        if (student == null) {
        ...
        Grade grade = Grade.create(subject, gradeScore, student.getId(), conn);
